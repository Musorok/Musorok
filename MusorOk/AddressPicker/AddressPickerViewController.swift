...
final class AddressPickerViewController: UIViewController, CLLocationManagerDelegate, YMKMapCameraListener {
    ...
    private let addressField = FormInputView(
        title: "Адрес, откуда забрать мусор",
...
//final class AddressPickerViewController: UIViewController, CLLocationManagerDelegate, YMKMapCameraListener {
//
//    private var areaOverlays: [YMKPolygonMapObject] = []
//
//    private struct Area {
//        let name: String
//        /// Наружный контур района (обход по часовой/против — не критично, первая и последняя точка могут не совпадать)
//        let outer: [CLLocationCoordinate2D]
//        /// Внутренние «дыры» (обычно не нужны) — оставь пустым
...
//    }
//
//    private var serviceAreas: [Area] {
//        return [
//            Area(name: "Орбита", outer: [
//                // TODO: Орбита (общий контур или объединённый многоугольник отдельных подрайонов)
//                CLLocationCoordinate2D(latitude: 43.200000, longitude: 76.850000),
//                CLLocationCoordinate2D(latitude: 43.205000, longitude: 76.860000),
//                CLLocationCoordinate2D(latitude: 43.198000, longitude: 76.870000),
//            ]),
//            Area(name: "Коктем", outer: [
//                // TODO: Коктем (1/2 можно слить в один контур, если соприкасаются, или рисовать два Area)
//                CLLocationCoordinate2D(latitude: 43.230000, longitude: 76.910000),
//                CLLocationCoordinate2D(latitude: 43.235000, longitude: 76.920000),
//                CLLocationCoordinate2D(latitude: 43.228000, longitude: 76.930000),
//            ]),
//            Area(name: "Алмагуль", outer: [
//                // TODO: Алмагуль
//                CLLocationCoordinate2D(latitude: 43.240000, longitude: 76.880000),
//                CLLocationCoordinate2D(latitude: 43.245000, longitude: 76.890000),
//                CLLocationCoordinate2D(latitude: 43.238000, longitude: 76.900000),
//            ])
//        ]
//    }
//
...
//    }
//
//    /// Преобразование массива координат в YMKLinearRing
//    private func makeRing(coords: [CLLocationCoordinate2D]) -> YMKLinearRing? {
//        guard coords.count >= 3 else { return nil }
//        let points = coords.map { YMKPoint(latitude: $0.latitude, longitude: $0.longitude) }
//        return YMKLinearRing(points: points)
//    }
//
//    /// Стиль заливки как на примере: зелёная обводка + полупрозрачная заливка
//    private func styleArea(_ obj: YMKPolygonMapObject) {
//        obj.strokeColor = UIColor.brandGreen            // твой фирменный зелёный
//        obj.strokeWidth = 4.0
//        obj.fillColor = UIColor.brandGreen.withAlphaComponent(0.18)
//        obj.zIndex = 10
//    }
//
//    /// Удалить старые и перерисовать районы
//    private func drawServiceAreas(_ areas: [Area]) {
//        // снять предыдущие
//        areaOverlays.forEach { $0.parent.remove(with: $0) }
//        areaOverlays.removeAll()
//
//        let mapObjects = ymapView.mapWindow.map.mapObjects
//        for area in areas {
//            guard let outerRing = makeRing(coords: area.outer) else { continue }
//            let innerRings: [YMKLinearRing] = area.holes.compactMap { makeRing(coords: $0) }
//            let polygon = YMKPolygon(outerRing: outerRing, innerRings: innerRings)
//            let obj = mapObjects.addPolygon(with: polygon)
//            styleArea(obj)
//            obj.userData = area.name
//            areaOverlays.append(obj)
//        }
//    }
//
//    // MARK: - Actions
